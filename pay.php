...
if ($login == true) {
    ...
    $cards = $conn->query("SELECT * FROM cards WHERE email = '$email'")->fetchAll();
    $vehicles = $conn->query("SELECT * FROM vehicles WHERE email = '$email'")->fetchAll();
    $parks = $conn->query("SELECT * FROM parks")->fetchAll();
    $reservs = $conn->query("SELECT * FROM reservations")->fetchAll();
    ...
    <script>
        ...
        echo "const cards = ". json_encode($cards) . ";";
        echo "const parks = ". json_encode($parks) . ";";
        echo "const vehicles = ". json_encode($vehicles) . ";";
        echo "const reservs = ". json_encode($reservs) . ";";
        ?>
    </script>
    <script src="static/js/main.js"></script>
    <script>gotoPage("pay.html")</script>
    <?php
} else {
    ?>
    <div id="main-wrap">
